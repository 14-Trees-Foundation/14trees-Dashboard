...
const defaultMessage = `**Ahoy Captain!** ⚓  
I'm your Shipping AI assistant, ready to help with vessel performance and operational data.
`;
...
const ChatbotV2 = () => {
    ...
    const getBotResponse = async (userInput: string, history: Message[]): Promise<string> => {
        const resp = await apiClient.handleShippingQuery(userInput, history);
        return resp.output;
    };
    ...
    const handleUpload = async (params: Params) => {
        ...
        for (const file of params.files) {
            const uploadPromise = awsUtils.uploadFileToS3('shipping-docs', file, 'documents/' + date);
    ...
    const flow = {
        ...
        user: {
            message: async (params: Params) => {
                ...
                if (respLower.includes("successfully") || respLower.includes("completed")) {
                    await params.showToast("✅ Operation completed successfully!", 3000);
                }
        ...
        process_options: {
            ...
            path: async (params) => {
                ...
                await params.injectMessage("Navigating to your requested page!");
                setTimeout(() => {
        ...
        prompt_again: {
            message: "Would you like to query any other shipping data?",
            ...
            settings={
                {
                    ...
                    header: {
                        title: <div style={{ cursor: 'pointer', margin: '0px', paddingTop: '5px', fontSize: '16px', fontWeight: 'light' }}>CaptainAI</div>,
