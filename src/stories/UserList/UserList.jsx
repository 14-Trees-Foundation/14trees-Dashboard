	...
	const renderCompactCard = (item, type) => {
		...
		return (
			<div className={classes.compactCard} key={`${type}-${item.id}`}>
				...
				<div
					className={`${classes.treeSections} ${
						hasPersonal && hasSponsored ? '' : classes.treeSectionsCentered
					}`}
				>
					{hasPersonal && (
						<div className={classes.treeSection}>
							<div className={classes.treeSectionLabel}>Personal</div>
							<div className={classes.treeSectionCount}>
								{item.assigned_trees_count}
							</div>
							<Button
								variant="outlined"
								color="success"
								...
								fullWidth
								onClick={() =>
									window.open(`${baseUrl}/profile/user/${item.id}`)
								}
					...
					{hasSponsored && (
						<div className={classes.treeSection}>
							...
							<Button
								...
								onClick={() =>
									window.open(
										`${baseUrl}/${isUser ? 'sponsorship' : 'group'}/${item.id}`,
									)
								}
								className={classes.treeButton}
							>
