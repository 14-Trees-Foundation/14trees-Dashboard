...
import {
	Event,
	EventMessage,
	EventMessageCreationAttributes,
...
class EventsApiClient {
	...
	async getEvents(
		...
		try {
			const response = await this.api.post<PaginatedResponse<Event>>(url, {
				filters: filters,
			});
	...
	async createEvent(
		...
		try {
			...
			if (eventData.tags && typeof eventData.tags === 'string') {
				...
				tagsArray = (eventData.tags as any)
					.split(',')
			...
			if ((images && images.length > 0) || eventPoster) {
				...
				if ((eventData as any).location) {
					formData.append(
						'location',
						JSON.stringify((eventData as any).location),
					);
				}
				...
				if ((eventData as any).theme_color)
					formData.append('theme_color', (eventData as any).theme_color);
	...
	async updateEvent(eventData: any): Promise<Event> {
		try {
			...
			const hasNewPoster =
				eventData.event_poster && eventData.event_poster instanceof File;
			// Check if there's a new landing image File as well
			const hasNewLanding =
				eventData.landing_image && eventData.landing_image instanceof File;
			...
			const hasNewLandingMobile =
				eventData.landing_image_mobile &&
				eventData.landing_image_mobile instanceof File;
			...
			if (
				...
				if (eventData.assigned_by)
					formData.append('assigned_by', eventData.assigned_by.toString());
				if (eventData.site_id)
					formData.append('site_id', eventData.site_id.toString());
				if (eventData.description)
					formData.append('description', eventData.description);
				...
				if (
					eventData.landing_image &&
					eventData.landing_image instanceof File
				) {
				...
				const response = await this.api.put<Event>(
					`/events/${eventData.id}`,
					formData,
				);
				return response.data;
	...
	async dissociateTreesFromEvent(
		eventId: number,
		treeIds: number[],
	): Promise<void> {
		try {
	...
	async getEventImages(eventId: number): Promise<EventImage[]> {
		try {
			const response = await this.api.get<EventImage[]>(
	...
	async uploadEventImages(
		...
		try {
			...
			const response = await this.api.post<EventImage[]>(
				`/events/${eventId}/images`,
				formData,
				{
					headers: {
	...
	async removeEventImages(eventId: number, imageIds: number[]): Promise<void> {
		try {
			await this.api.delete(`/events/${eventId}/images`, {
				data: { imageIds },
			});
		} catch (error: any) {
			if (error.response?.data?.message) {
	...
	async reorderEventImages(
		eventId: number,
		imageSequences: ImageSequenceUpdate[],
	): Promise<void> {
		try {
			await this.api.put(`/events/${eventId}/images/reorder`, {
				imageSequences,
			});
		} catch (error: any) {
			if (error.response?.data?.message) {
	...
	async getEventMessages(event_link: string): Promise<EventMessage[]> {
		try {
			const response = await this.api.get<EventMessage[]>(
				`/events/messages/${event_link}`,
			);
			return response.data;
		} catch (error: any) {
			if (error.response?.data?.message) {
	...
	async createEventMessage(
		...
		try {
			...
			const response = await this.api.post<EventMessage>(
	...
	async updateEventMessage(
		...
		try {
			...
			const response = await this.api.put<EventMessage>(
				`/events/messages/${messageId}`,
				payload,
			);
			return response.data;
		} catch (error: any) {
	...
	async reorderEventMessages(
		eventId: number,
		messageSequences: MessageSequenceUpdate[],
	): Promise<void> {
		try {
			await this.api.put(`/events/${eventId}/messages/reorder`, {
				messageSequences,
			});
		} catch (error: any) {
			if (error.response?.data?.message) {
	...
	async getEventLandingData(linkId: string): Promise<EventLandingData> {
		try {
			const response = await this.api.get<EventLandingData>(
				`/events/by-link/${encodeURIComponent(linkId)}`,
			);
			return response.data;
		} catch (error: any) {
			if (error.response?.data?.message) {
