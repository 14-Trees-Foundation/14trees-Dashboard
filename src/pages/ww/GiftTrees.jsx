...
export const GiftTrees = () => {
	...
	const assignTrees2 = async (
		...
		if (sponsorUser && sponsorUser.id)
			formData.append('sponsored_by_user', sponsorUser.id);
		if (sponsorGroup && sponsorGroup.id)
			formData.append('sponsored_by_group', sponsorGroup.id);
	...
	if (values.loading) {
		...
		if (
			!group_id &&
			(!values.user || Object.keys(values.user).length === 0) &&
			!values.loading
		) {
			return (
				<Typography
                  ...
                  <div style={{ margin: "0px", fontWeight: "bold" }}>
                    Team, 14Trees Foundation
						...
						<div
							...
							style={{ paddingTop: '16px', paddingBottom: '2px' }}
						>
							<Typography
								variant="h4"
								align="left"
						...
						<div className={classes.itembox} style={{ paddingTop: '8px' }}>
							<Box>
								<Typography
									variant="h4"
									align="left"
									...
									<TableHead>
										<TableRow sx={{ fontSize: '16px' }}>
											...
											<TableCell align="left">
												<Checkbox
													checked={values.filteredTrees.every(
														(item) => item.selected,
													)}
													onClick={(e) => handleSelectAlltree(e)}
												/>
											</TableCell>
											<TableCell align="right"></TableCell>
