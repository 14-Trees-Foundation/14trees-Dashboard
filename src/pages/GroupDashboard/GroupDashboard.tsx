...
const GROUP_IMAGES: Record<string, string> = {
	'96': buildS3Url('groups/Skoda_ai.png'),
	'82': buildS3Url('groups/Yardi.png'),
	// Add more group-specific images here as needed
};

const DEFAULT_GROUP_IMAGE =
	'https://via.placeholder.com/800x600/4CAF50/FFFFFF?text=14+Trees+Partnership';
...
const GroupDashboard: React.FC = () => {
	...
	const handlePrevVisit = () => {
		setCurrentVisitIndex(
			(prev) => (prev - 1 + visitsData.length) % visitsData.length,
		);
	};
			...
			<Box className={classes.contentArea}>
						...
						<Grid item xs={12} md={5}>
							<Box className={classes.heroImageContainer}>
								<img
									src={
										groupId
											? GROUP_IMAGES[groupId] || DEFAULT_GROUP_IMAGE
											: DEFAULT_GROUP_IMAGE
									}
									alt="Partnership"
						...
						<Grid item xs={12} md={7}>
							<Box className={classes.welcomeText}>
								...
								<Typography variant="body1" className={classes.welcomeBody}>
									At 14 Trees Foundation, we are deeply grateful for your
									commitment to environmental stewardship. Together, we are
									creating sustainable ecosystems, restoring degraded lands, and
									providing livelihoods to local communities. Your support helps
									us plant native trees, conserve biodiversity, and build a
						...
						<Grid item xs={6} sm={6} md={4} lg={3}>
							<Card className={classes.metricCard}>
								<CardContent>
									...
									>
										{groupInfo?.trees_planted?.toLocaleString() ||
											groupStats.trees.sponsored_trees.toLocaleString()}
									</Typography>
				...
				{visitsData.length > 0 && (
					<Box sx={{ mt: 4 }}>
						...
						<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
							Thank you for visiting our reforestation sites and witnessing the
							impact firsthand.
						</Typography>
				...
				{eventsData.length > 0 && (
					<Box sx={{ mt: 4, mb: 3 }}>
						...
						<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
							Groves are a great way to anchor memories in the trees planted.
							Here are the groves you’ve created in the forest.
						</Typography>
						<Grid container spacing={2}>
