...
const EVENT_DASHBOARD_CONFIG_BY_LINK_ID: Record<
	string,
	EventDashboardLinkConfig
> = {
	jjaqyf4c: {
...
const EventDashboard: React.FC<EventDashboardProps> = ({
	...
	const [eventTreeTypesCount, setEventTreeTypesCount] = useState<number | null>(
		null,
	);
	const [eventTreeTypes, setEventTreeTypes] = useState<
		Array<{ label: string; illustration?: string }>
	>([]);

	const apiClient = new ApiClient();
	const linkConfig = EVENT_DASHBOARD_CONFIG_BY_LINK_ID[event.link ?? ''] ?? {};
	const shouldRenderFeaturedImage = Boolean(
		linkConfig.showFeaturedImage && linkConfig.featuredImageSrc,
	);
	...
	const featuredImageAlt =
		linkConfig.featuredImageAlt ?? `${event.name} featured`;
	const memoriesHeadingText =
		linkConfig.memoriesHeadingText ??
		(event.type === '2' ? 'Memories' : 'Event Memories');
	const memoriesHeadingColor = linkConfig.memoriesHeadingColor;
	const memoriesHeadingAlign =
		linkConfig.memoriesHeadingAlign ?? (isMobile ? 'center' : 'left');
	const treesHeadingText =
		linkConfig.treesHeadingText ??
	...
	const getLandingImage = (event_id: number) => {
		switch (event_id) {
			case 8245:
				return isMobile
					? buildS3Url(
							'events/landing_images/aayushi_wedding_landing_page_poster_phone.png',
					  )
					: buildS3Url(
							'events/landing_images/aayushi_wedding_landing_page_poster.jpg',
					  );
			case 8251:
				return buildS3Url(
					'events/landing_images/jayasree_adhiraj_wedding_grove.jpg',
				);
			// default:
			//     return buildS3Url("events/default.jpg");
		}
	};
	// Prefer the server-provided landing image if available, otherwise fall back to id-based mapping
	const landingImageUrl =
		isMobile && event.landing_image_mobile_s3_path
	...
	const currentTheme =
		event.theme_color && themeConfigs[event.theme_color]
	...
	const logoBackgroundColor =
		currentTheme.logoColor ??
		currentTheme.gradient?.split(',')[0].split('(')[1].trim();

	useEffect(() => {
		const fetchEventImages = async () => {
			try {
				setIsLoadingImages(true);
				const images: EventImage[] = await apiClient.events.getEventImages(
					event.id,
				);
	...
	useEffect(() => {
		...
		let legacyImages = [
			...
			return buildS3Url(`memories/memory${number}.jpg`);
		});
		...
		const fetchTreeTypes = async () => {
			try {
				...
				const mapped = results.map((r) => ({
					label: String(r.plant_type || ''),
					illustration: r.info_card_s3_path || undefined,
				}));
	...
	const memoryImages =
		event.event_poster && allEventImages.length > 0
	...
	const [validatedSpeciesImages, setValidatedSpeciesImages] = useState<
	...
	const species: Array<{ src: string; label: string }> = (() => {
		const base =
			validatedSpeciesImages.length > 0
				...
				>
					...
					>
						...
						>
							...
							>
								14 Trees Foundation is a non-profit organisation that believes
								in a holistic effort for reforestation. We work on three
								parallels- native ecological revival, employing local people and
								increasing money flow to rural areas, and bridging the gap
								between urban dwellers and forests.
								<br />
								<br />
								We believe that a forest can only stand the test of time if
								these three pillars are aligned.
								<br />
								<br />
								We're thrilled to create a grove for you celebrating your
								special day!
								<br />
								<br />
		...
		>
			...
			{!shouldHideHeader && (
				<>
					...
					>
						<img
							src={event.link === 'ij5h8ow9' ? greenSprihLogo : logo}
							alt={
								event.link === 'ij5h8ow9' ? 'GreenSprih Logo' : '14Trees Logo'
							}
