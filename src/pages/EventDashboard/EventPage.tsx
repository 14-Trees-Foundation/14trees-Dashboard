...
async function getEventDetails(linkId: string) {
	try {
		...
		const event = resp.results.length === 1 ? resp.results[0] : null;
...
const EventPage: React.FC = () => {
	...
	const backgroundColor =
		linkId && BACKGROUND_COLOR_BY_LINK_ID[linkId]
			? BACKGROUND_COLOR_BY_LINK_ID[linkId]
			: DEFAULT_BACKGROUND_COLOR;
	const shouldShowDrawer =
		!isMobile &&
		((linkId ? SHOW_DRAWER_BY_LINK_ID[linkId] : undefined) ??
			DEFAULT_SHOW_DRAWER);
	...
	const mainBox = () => {
		...
		return (
			<div>{event && <Page event={event} eventMessages={eventMessages} />}</div>
		);
	};
	...
	return loading ? (
		...
		<Box style={{ display: 'flex', backgroundColor }}>
			...
			<Box style={{ padding: isMobile ? 0 : 0, flexGrow: 1 }}>{mainBox()}</Box>
