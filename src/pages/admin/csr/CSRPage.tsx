...
const CSRPage: React.FC = () => {
	...
	const items = [
		...
		},
		{
			...
			logo: CardGiftcard,
			key: 1,
			display: status.view_name !== 'Infineon Dashboard',
					...
					>
						...
						>
							<CSRHeader
								groupId={groupId}
								onGroupChange={(group) => {
									setCurrentGroup((prev) => (prev ? prev : group));
								}}
							/>
							<Box sx={{ p: 2 }}>
								{activeTab === 'greenTributeWall' && currentGroup && (
									<CSRInventory selectedGroup={currentGroup} />
								)}
								{activeTab === 'prePurchase' && currentGroup && (
									<CSRGiftRequests
										selectedGroup={currentGroup}
										groupId={currentGroup.id}
									/>
								)}
								{activeTab === 'giftHistory' && currentGroup && (
									<CSRGiftHistory
										selectedGroup={currentGroup}
										groupId={currentGroup.id}
									/>
								)}
								{activeTab === 'donations' && currentGroup && (
									<CSRDonations selectedGroup={currentGroup} />
								)}
								{activeTab === 'Setting-Details' && currentGroup && (
									<CSRSettings
										group={currentGroup}
										onGroupChange={(group) => {
