...
export const SitesComponent = () => {
	...
	const getSiteData = async () => {
		...
		const categoryIdx = filtersData.findIndex(
			(item) => item.columnField === 'category',
		);
		if (categoryIdx > -1) {
			filtersData[categoryIdx].value = (
				filtersData[categoryIdx].value as string[]
			...
			render: (value, record, index) => {
				return value ? (
					<Button variant="outlined" color="success">
						<a
							href={value}
							download={record.name_english + '.kml'}
          ...
          >
            Sync Notion Sites
					...
					<AddSiteForm
						...
						onSubmit={async (data) => {
							await createSite(data as any, undefined as any);
							getSiteData();
						}}
					/>
				</div>
