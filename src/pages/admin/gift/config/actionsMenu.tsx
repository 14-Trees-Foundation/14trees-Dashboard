...
interface ActionsMenuProps {
	...
	handlers: {
		...
		handleOpenPlotSelection: (record: GiftCard) => void;
...
export const createActionsMenu = ({
	...
	const items: MenuProps['items'] = [
		...
		...(record.request_type !== 'Visit' &&
		record.request_type !== 'Normal Assignment'
			? [
					{
						key: '51',
		...
		...(record.validation_errors === null ||
		!record.validation_errors.includes('MISSING_USER_DETAILS')
			? [
					{
						key: '11',
		...
		record.status === 'pending_gift_cards' ||
		record.booked > 0
			? [
					{ type: 'divider' },
					...(record.booked > 0
						...
						? [
								{
									...
									onClick: () =>
										handleDownloadCards(
											record.id,
											record.user_name + '_' + record.no_of_cards,
											'zip',
										),
								},
						  ]
		...
		...(!auth.roles.includes(UserRoles.User)
			? [
					{ type: 'divider' },
					{
						...
						label: 'Reserve Trees',
						icon: <Landscape />,
						onClick: () => handleOpenPlotSelection(record),
					},
					{
						key: '41',
						label: 'Assign Trees',
