...
import { GiftCard, PlotAutoBookRule } from '../../../../types/gift_card';
...
export const useGiftCardState = () => {
	...
	const plotAutoBookRulesRef = useRef<PlotAutoBookRule[]>([]);
	const plotAutoBookValidationErrorRef = useRef<string | null>(null);
	const [selectedPaymentGR, setSelectedPaymentGR] = useState<GiftCard | null>(
		null,
	);
	const [selectedGiftRequestIds, setSelectedGiftRequestIds] = useState<
		number[]
	>([]);
	...
	return {
		...
		selectedGiftRequestIds,
		setSelectedGiftRequestIds,
		album,
		setAlbum,
		tags,
		setTags,
