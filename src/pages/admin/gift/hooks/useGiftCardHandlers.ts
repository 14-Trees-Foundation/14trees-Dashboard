...
import {
	GiftCard,
	GiftRequestType_CARDS_REQUEST,
	GiftRequestType_NORAML_ASSIGNMENT,
	GiftRequestUser,
	PlotAutoBookRule,
	SponsorshipType,
...
export const useGiftCardHandlers = (props: UseGiftCardHandlersProps) => {
	...
	const handleAlbumSave = async (files: (File | string)[]) => {
		...
		try {
			if (album) {
				...
				await apiClient.updateAlbumImagesForGiftRequest(
					selectedGiftCard.id,
					album.id,
				);
			} else {
	...
	const handleOpenPlotSelection = async (record: GiftCard) => {
		try {
			const apiClient = new ApiClient();
			const response = await apiClient.getGiftCards(0, 1, [
				{ columnField: 'id', operatorValue: 'equals', value: record.id },
			]);
			const latestRecord = response.results[0] ?? record;
			setBookNonGiftable(
				latestRecord.request_type === GiftRequestType_NORAML_ASSIGNMENT ||
					latestRecord.request_type === 'Visit',
			);
			setSelectedGiftCard(latestRecord);
			setPlotModal(true);
		} catch (error: any) {
			toast.error(error.message || 'Failed to load latest request details');
		}
	};
	...
	const handlePlotSelectionCancel = () => {
		...
		setSelectedTrees([]);
		plotAutoBookRulesRef.current = [];
		plotAutoBookValidationErrorRef.current = null;
	};

	const handlePlotSelectionSubmit = async () => {
		if (!selectedGiftCard) return;

		if (selectedTrees.length === 0 && plotAutoBookValidationErrorRef.current) {
			toast.error(plotAutoBookValidationErrorRef.current);
			return;
		}
		...
		if (selectedPlots.length !== 0) {
			try {
				await apiClient.createGiftCardPlots(
					selectedGiftCard.id,
				...
				await apiClient.bookGiftCards(
					...
					diversify,
					bookAllHabits,
					selectedTrees.length === 0 ? plotAutoBookRulesRef.current : undefined,
				);
				...
				const errorMessage =
					error.message || 'Failed to book trees for gift card request';
