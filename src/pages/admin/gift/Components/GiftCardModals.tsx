...
export const GiftCardModals: React.FC<GiftCardModalsProps> = ({
	...
	const selectedRequestId = selectedGiftCard?.id;
	const selectedPaymentRequestId = selectedPaymentGR?.id;
	...
	return (
		<>
			...
			>
				<DialogTitle>
					{getDialogTitleWithReqId('Reserve Trees', selectedRequestId)}
				</DialogTitle>
				<DialogContent dividers>
					{selectedGiftCard && (
						<PlotSelection
							giftCardRequestId={selectedGiftCard.id}
							onTreeSelection={(trees: any[]) => {
								setSelectedTrees(trees);
							}}
							totalTrees={selectedGiftCard.no_of_cards}
							requiredTrees={
								selectedGiftCard.no_of_cards - Number(selectedGiftCard.booked)
							}
							plots={selectedPlots}
							onPlotsChange={(plots) => setSelectedPlots(plots)}
							bookNonGiftable={bookNonGiftable}
							onBookNonGiftableChange={(value) => {
								setBookNonGiftable(value);
							}}
							diversify={diversify}
							onDiversifyChange={(value) => {
								setDiversify(value);
							}}
				...
				<DialogActions>
					<Button
						onClick={handlePlotSelectionCancel}
						color="error"
						variant="outlined"
					>
						Cancel
			...
			>
				<DialogTitle>
					{getDialogTitleWithReqId(
						'Delete tree cards request',
						selectedRequestId,
					)}
				</DialogTitle>
				<DialogContent dividers>
					...
					<Typography variant="body1">
						This action will unassing and unreserve all the trees from this
						request. It will also delete any payment/donation details as well.
					</Typography>
				</DialogContent>
				<DialogActions>
					<Button
					...
					<Button
						onClick={handleGiftCardRequestDelete}
						color="error"
						variant="contained"
					>
						Delete
			...
			<Dialog open={paymentModal} fullWidth maxWidth="xl">
				...
				<DialogContent dividers>
					<PaymentComponent
						initialAmount={
							(selectedPaymentGR?.no_of_cards || 0) *
							(selectedPaymentGR?.category === 'Foundation'
								? 3000
								: selectedPaymentGR?.request_type === 'Normal Assignment'
								? 1500
								: 2000)
						}
