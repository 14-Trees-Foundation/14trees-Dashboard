...
const GiftCardRequestInfo: React.FC<GiftCardRequestInfoProps> = ({
	open,
	onClose,
	data,
}) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				const resp = await apiClient.getBookedGiftTreesWithQueryFilters(
					data.id,
					0,
					10,
					[],
				...
				console.error(
					'Failed to fetch booked trees (GET):',
					error?.message || error,
				);
			}
					...
					<Button
						...
						onClick={() => {
							...
							if (hostname === 'localhost' || hostname === '127.0.0.1') {
								...
								window.open(
									'https://' + hostname + '/profile/' + record.sapling_id,
								);
							}
						}}
					>
	...
	return (
		<Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
			<DialogTitle>
				{getDialogTitleWithReqId('Request summary', data?.id)}
			</DialogTitle>
			<DialogContent dividers>
				...
				{data.group_name && (
					<Box mt={2} mb={2}>
						...
						<Typography>
							<strong>Corporate:</strong> {data.group_name || 'N/A'}
						</Typography>
						<Box display="flex">
							<Typography>
								<strong>Logo:</strong>
				...
				<Box mt={2} mb={2}>
					...
					<Typography>
						<strong>Event Name:</strong> {data.event_name || 'N/A'}
					</Typography>
					<Typography>
						<strong>Gifted By:</strong> {data.planted_by || 'N/A'}
					</Typography>
					<Typography>
						<strong>Total No. of Cards Requested:</strong> {data.no_of_cards}
					</Typography>
					<Typography>
				...
				<Box mt={2} mb={2}>
					...
					<GeneralTable
						...
						onPaginationChange={(newPage: number, newPageSize: number) => {
							setBookedPage(newPage - 1);
							setBookedPageSize(newPageSize);
						}}
						onDownload={async () => {
							const apiClient = new ApiClient();
							try {
