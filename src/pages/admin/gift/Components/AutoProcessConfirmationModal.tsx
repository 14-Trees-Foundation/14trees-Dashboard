...
const AutoProcessConfirmationModal: React.FC<
	AutoProcessConfirmationModalProps
> = ({ open, onClose, onConfirm, loading = false, giftId, treesToBook }) => {
	...
	useEffect(() => {
		const fetchPlotDetails = async () => {
			...
			try {
				...
				const details =
					await apiClient.getPlotTreesCountForAutoProcessGiftRequest(giftId);
	...
	const totalTrees = plotDetails.reduce(
		(sum, plot) => sum + plot.trees_count,
		0,
	);
	...
	return (
		<Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
			<DialogTitle>
				{getDialogTitleWithReqId('Auto Process Confirmation', giftId)}
			</DialogTitle>
			<DialogContent dividers>
				<Box sx={{ mb: 2 }}>
					<Typography variant="body1" gutterBottom>
						This request will be automatically processed with the following
						steps:
					</Typography>
				...
				{isLoading ? (
					...
					<DialogContentText color="warning.main">
						No plots available for auto-processing. Please select plots
						manually.
					</DialogContentText>
				) : plotDetails.length > 0 && treesToBook > 0 ? (
					<Box>
