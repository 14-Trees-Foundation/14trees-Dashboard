...
const EditUserDialog: React.FC<EditUserDialogProps> = ({
	...
	const handleAssigneeEmailChange = (
		event: React.SyntheticEvent,
		value: string,
	) => {
	...
	const handleEmailChange = (
		event: React.SyntheticEvent,
		value: string,
		field: 'recipient_email' | 'assignee_email',
	) => {
		let isSet = false;
	...
	return (
		<Dialog open={open} fullWidth maxWidth="md">
			<DialogTitle>
				{getDialogTitleWithReqId('Edit Recipient details', requestId)}
			</DialogTitle>
...
const EditUserDetailsModal: React.FC<EditUserDetailsModalProps> = ({
	...
	const [selectedUser, setSelectedUser] = useState<GiftRequestUser | null>(
		null,
	);
		...
		const getUsers = async () => {
			...
			setUserList(
				users
					...
					.filter(
						(item, idx, self) =>
							self.findIndex((card) => card.assignee === item.assignee) === idx,
					)
	...
	const handleUserEdit = (row: GiftRequestUser, image?: File) => {
		const saveImage = async () => {
			if (image && requestId) {
				...
				const location = await awsUtils.uploadFileToS3(
					'gift-request',
					image,
					requestId,
				);
	...
	return (
		<Dialog open={open} fullWidth maxWidth="lg">
			<DialogTitle>
				{getDialogTitleWithReqId('Recipients List', reqNumber ?? giftRequestId)}
			</DialogTitle>
			<DialogContent>
				<GeneralTable
					loading={false}
				...
				{selectedUser && (
					<EditUserDialog
						open={editModal}
						requestId={reqNumber ?? giftRequestId}
						imageUrls={imageUrls}
