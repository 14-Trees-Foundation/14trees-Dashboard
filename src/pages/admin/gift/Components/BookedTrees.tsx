...
const BookedTrees: React.FC<BookedTreesProps> = ({
	giftCardRequestId,
	visible,
	onUnMap,
}) => {
	...
	const [existingBookedTrees, setExistingBookedTrees] = useState<
		GiftCardUser[]
	>([]);
	...
	const getBookedTrees = async (giftRequestId: number) => {
		...
		try {
			...
			const bookedTreesResp =
				await apiClient.getBookedGiftTreesWithQueryFilters(
					giftRequestId,
					0,
					-1,
			...
			setExistingBookedTrees(
				bookedTreesResp.results.map((item) => ({ ...item, key: item.id })),
			);
	...
	const columns: TableColumnsType<GiftCardUser> = [
		...
		},
		{
			...
			align: 'center',
			width: 200,
			...getColumnSearchProps('plot_name', filters, handleSetFilters, true),
		},
		{
	...
	const handleUnMapTrees = async (unMapAll: boolean = false) => {
		try {
			const treeIds: number[] = existingBookedTrees
				.filter((tree) => selectedIds.some((id) => id === tree.id))
				.map((item) => item.tree_id);
			const apiClient = new ApiClient();
			await apiClient.unBookGiftTrees(
				giftCardRequestId,
				unMapAll ? [] : treeIds,
				unMapAll,
			);
			...
			onUnMap &&
				onUnMap(unMapAll ? existingBookedTrees.length : treeIds.length);
	...
	return (
		<Box hidden={totalBookedTrees > 0 ? false : !visible}>
			<Box
				display="flex"
				alignItems="center"
			...
			>
				<Typography variant="h6" flexGrow={1}>
					Previously booked trees:
				</Typography>
				<Button
					variant="contained"
					color="success"
					onClick={() => {
						setUnMapAllConfirmation(true);
					}}
				>
					Unreserve All Trees
				</Button>
			...
			<Dialog open={unMapConfirmation} maxWidth="md">
				...
				<DialogContent dividers>
					<Typography>
						Are you sure you want to unreserve selected trees? This action will
						also unassign trees if they are assigned to some users.
					</Typography>
				</DialogContent>
				<DialogActions>
					<Button
						onClick={() => {
							setUnMapConfirmation(false);
						}}
						color="error"
						variant="outlined"
					>
			...
			<Dialog open={unMapAllConfirmation} maxWidth="md">
				...
				<DialogContent dividers>
					<Typography>
						Are you sure you want to unmap <strong>all the trees</strong> for
						this gift request? This action will also unassign trees if they are
