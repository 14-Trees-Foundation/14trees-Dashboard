...
const AssignTrees: React.FC<AssignTreesProps> = ({
	giftCardRequestId,
	requestId,
	open,
	onClose,
		...
		<Dialog open={open} fullWidth maxWidth="lg">
			<DialogTitle>
				{getDialogTitleWithReqId(
					'Assign trees to tree card request recipients',
					requestId ?? giftCardRequestId,
				)}
			</DialogTitle>
				...
				<Box>
					...
					>
						...
						<ToggleButtonGroup
							...
							exclusive
							onChange={(e, value) => {
								setAutoAssign(value === 'yes' ? true : false);
							}}
							aria-label="Platform"
							size="small"
						>
					...
					<Modal
						...
						aria-labelledby="tree-select-modal-title"
					>
						<Box
							sx={{
								position: 'absolute',
						...
						>
							<GeneralTable
								rows={users.filter(
									(user) =>
										trees.filter(
											(tree) => tree.gift_request_user_id === user.id,
										).length < user.gifted_trees,
								)}
								columns={userColumns}
								totalRecords={
									users.filter(
