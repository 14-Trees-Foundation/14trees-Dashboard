...
const EmailConfirmationModal: React.FC<EmailConfirmationModalProps> = ({
	...
	const [receiverCC, setReceiverCC] = useState<string[]>(
	...
	useEffect(() => {
		const getEmailTemplates = async () => {
			...
			const uniqueTemplates: EmailTemplate[] = templates.filter(
				(template, index, self) =>
					self.findIndex((item) => item.event_type === template.event_type) ===
					index,
			);

			setEmailTemplates(uniqueTemplates);
			setSelectedTemplate(
				uniqueTemplates.find((template) => template.event_type === 'default') ??
					null,
			);
		};
	...
	return (
		<Dialog open={open} fullWidth maxWidth="lg">
			<DialogTitle>
				{getDialogTitleWithReqId('Send Emails', requestId)}
			</DialogTitle>
			<DialogContent dividers>
					...
					<FormControl component="fieldset">
						<FormGroup aria-label="position" row>
							<FormControlLabel
								...
								control={
									<Checkbox
										checked={emailReceiver}
										onChange={(e) => {
											setEmailReceiver(e.target.checked);
										}}
									/>
								}
								label="Receivers"
							...
							<FormControlLabel
								...
								control={
									<Checkbox
										checked={emailSponsor}
										onChange={(e) => {
											setEmailSponsor(e.target.checked);
										}}
									/>
								}
								label="Sponsor"
							...
							<FormControlLabel
								...
								control={
									<Checkbox
										checked={emailAssignee}
										onChange={(e) => {
											setEmailAssignee(e.target.checked);
										}}
									/>
								}
								label="Assignees"
				...
				<Box mt={3}>
					...
					<FormControl fullWidth>
						<OutlinedInput
							...
							startAdornment={sponsorCC.map((email, index) => (
								<Chip
									key={index}
									label={email}
				...
				<Box mt={3}>
					...
					<FormControl fullWidth>
						<OutlinedInput
							...
							startAdornment={receiverCC.map((email, index) => (
								<Chip
									key={index}
									label={email}
				...
				<Box sx={{ mt: 2 }}>
					<Typography>
						You can select different type of email template below.
					</Typography>
					<Autocomplete
				...
				<FormControlLabel
					...
					control={
						<Checkbox
							checked={attachCards}
							onChange={() => setAttachCards(!attachCards)}
						/>
					}
				...
				<Typography variant="body1" gutterBottom sx={{ mt: 2 }}>
					In case, you want to check emails first, enter your email address
					below in order to receive test emails.
				</Typography>
				...
				<Box display="flex" alignItems="center" justifyContent="space-between">
					<FormControl fullWidth>
						<OutlinedInput
							...
							startAdornment={toEmails.map((email, index) => (
								<Chip
									key={index}
									label={email}
