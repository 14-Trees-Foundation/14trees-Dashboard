...
export const AdminLeftDrawer = () => {
	...
	useEffect(() => {
		...
		try {
			...
			const admin =
				rolesArr.includes(UserRoles.Admin) ||
				rolesArr.includes(UserRoles.SuperAdmin);
			setIsAdmin(admin);
			if (!admin) {
				navigate('/tree-cards');
			}
	...
	const handleSubSectionClick = (path, sectionId) => {
		...
		setTimeout(() => {
			...
			if (element) {
				...
				const scrollTo =
					absoluteElementTop - middleOfScreen + elementRect.height / 2;
						...
						return (
							<div key={i}>
								...
								<div className={classes.item}>
									<div
										className={
											isActive(item.path) ? classes.selected : classes.itembtn
										}
...
const useStyles = makeStyles((theme) =>
	createStyles({
		...
		selected: {
			...
			justifyContent: 'flex-start',
			boxShadow:
				'7px 7px 11px #0d1710,-7px -7px 11px #31553a, inset 0 1px 0 rgba(255, 255, 255, 0.2)',
		...
		subSelected: {
			...
			boxShadow:
				'0 2px 4px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.2), 0 0 8px rgba(76, 175, 80, 0.3)',
