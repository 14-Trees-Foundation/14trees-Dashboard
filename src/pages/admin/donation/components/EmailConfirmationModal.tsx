					...
					<FormControl component="fieldset">
						<FormGroup aria-label="position" row>
							<FormControlLabel
								...
								control={
									<Checkbox
										checked={email_recipient}
										onChange={(e) => {
											setEmailRecipient(e.target.checked);
										}}
									/>
								}
								label="Recipients"
								...
								onKeyDown={(e) => {
									if (e.key === 'Enter') {
										...
										handleAddEmail(
											recipient_cc_input,
											setRecipientCcMails,
											setRecipient_cc_input,
										);
									}
								}}
								startAdornment={recipient_cc_mails.map((email, index) => (
									<Chip
										key={index}
										label={email}
										onDelete={() =>
											handleDeleteEmail(email, setRecipientCcMails)
										}
										color="secondary"
										sx={{ marginRight: 0.5 }}
									/>
					...
					<Box sx={{ mt: 2 }}>
						...
						<FormControl fullWidth>
							<OutlinedInput
								...
								startAdornment={sponsor_cc_mails.map((email, index) => (
									<Chip
										key={index}
										label={email}
					...
					<Box sx={{ mt: 2 }}>
						...
						<FormControl fullWidth>
							<OutlinedInput
								...
								startAdornment={assignee_cc_mails.map((email, index) => (
									<Chip
										key={index}
										label={email}
										onDelete={() =>
											handleDeleteEmail(email, setAssigneeCcMails)
										}
										color="secondary"
										sx={{ marginRight: 0.5 }}
									/>
				...
				<Box display="flex" alignItems="center" justifyContent="space-between">
					<FormControl fullWidth>
						<OutlinedInput
							...
							startAdornment={test_mails.map((email, index) => (
								<Chip
									key={index}
									label={email}
			...
			<DialogActions>
				...
				<Button
					...
					disabled={
						isConfirmLoading ||
						test_mails.length > 0 ||
						test_email_input.trim().length > 0
					}
				>
