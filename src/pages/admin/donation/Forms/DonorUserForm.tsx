...
interface DonorUserFormProps {
    user: User | null;
    onSelect: (user: User | null) => void;
}

const DonorUserForm: FC<DonorUserFormProps> = ({ user, onSelect }) => {
    ...
    return (
        <Box>
            ...
            {formOption === 'new' && <Box>
                <Typography variant="body1" sx={{ mb: 1 }}>Add Donor details</Typography>
                <form onSubmit={handleCreateUserSubmit}>
                    <Grid container rowSpacing={2} columnSpacing={1}>
                        ...
                        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'center' }}>
                            <Button variant='contained' color='success' type="submit" sx={{ marginLeft: '10px' }}>Add Donor</Button>
                        </Grid>
                    </Grid>
                </form>
                <Typography variant="body1">Donor already exists? <Typography onClick={() => setFormOption('existing')} style={{ cursor: 'pointer' }} color='primary' variant="body1" component="span">Select Donor</Typography>.</Typography>
            </Box>}
        </Box>
    );
}

export default DonorUserForm;
