...
export const AdminHome = () => {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			// Fetch dashboard/profile visit counters.
			try {
				const visitStatsResponse = await Axios.default.get(
					`/analytics/page-visits/summary`,
					{
						headers: {
							'x-access-token': token,
							'content-type': 'application/json',
				...
				if (visitStatsResponse.status === 200) {
					const stats = visitStatsResponse.data || {};
					const totals = stats.totals || {};
					const fallbackTopUrls = Array.isArray(stats.results)
						? stats.results
						: [];
					const topUrls = Array.isArray(stats.top_urls)
						? stats.top_urls
						: fallbackTopUrls;
					...
					setVisitStats({
						totalHits:
							Number(
								stats.total_hits ??
									totals.all ??
									Number(totals.profile || 0) + Number(totals.dashboard || 0),
							) || 0,
						profileHits: Number(stats.profile_hits ?? totals.profile) || 0,
						dashboardHits:
							Number(stats.dashboard_hits ?? totals.dashboard) || 0,
						trackedUrls: Number(stats.tracked_urls ?? stats.url_count) || 0,
						topUrls,
					});
				}
			} catch (error) {
				// Keep admin dashboard usable even if visit analytics endpoint is not available.
			}
			...
			if (treesResponse.status === 200) {
				...
				if (Array.isArray(treesResponse.data)) {
					...
					console.warn(
						'Tree logged by date data is not an array:',
						treesResponse.data,
					);
	...
	const MetricCard = ({
		...
		<Grow in={true} timeout={1000 + delay}>
			<Card
				className={classes.enhancedCard}
				style={{
	...
	const visitTotalHits =
		visitStats.totalHits ||
		Number(adminSummary.totalPageHits || adminSummary.totalVisitHits || 0);
	const visitProfileHits =
		visitStats.profileHits || Number(adminSummary.profileUrlHits || 0);
	const visitDashboardHits =
		visitStats.dashboardHits || Number(adminSummary.dashboardUrlHits || 0);
	const visitTrackedUrls =
		visitStats.trackedUrls || Number(adminSummary.trackedUrlsCount || 0);
	const topPublicUrls =
		visitStats.topUrls.length > 0
			? visitStats.topUrls
			: Array.isArray(adminSummary.topPublicUrls)
			? adminSummary.topPublicUrls
			: [];
		...
		<div>
			<Grid container spacing={3} sx={{ p: 2 }}>
				...
				<Grid item xs={12}>
					<Typography
						variant="h5"
						sx={{ mb: 2, color: '#1565C0', fontWeight: 600 }}
					>
						🌍 Geographic Coverage
					</Typography>
					<Grid container spacing={2}>
				...
				{adminSummary?.landTypeCounts &&
					Object.keys(adminSummary.landTypeCounts).length > 0 && (
						<Grid item xs={12}>
							<Typography
								variant="h5"
								sx={{ mb: 2, color: '#388E3C', fontWeight: 600 }}
							>
								🌱 Land Type Distribution
							</Typography>
							<Grid container spacing={2}>
								{Object.entries(adminSummary.landTypeCounts).map(
									([key, value], index) => (
...
const useStyles = makeStyles((theme) =>
	createStyles({
		...
		enhancedCard: {
			...
			boxShadow:
				'inset 7px 7px 14px rgba(0,0,0,0.1), inset -7px -7px 14px rgba(255,255,255,0.3), 0 4px 12px rgba(0,0,0,0.1)',
