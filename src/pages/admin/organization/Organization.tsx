...
export const OrganizationComponent = () => {
	...
	const { getGroups, createGroup, updateGroup, deleteGroup } =
	...
	const [groupTreeCounts, setGroupTreeCounts] = useState<
		Record<
			number,
			{
				mapped_trees: number;
	...
	const handleSortingChange = (param: {
		field: string;
		order?: 'ASC' | 'DESC';
	}) => {
		if (!param.order) {
	...
	const getSortableHeader = (header: string, key: string) => {
		return (
			...
			>
				...
				{getSortIcon(
					key,
					orderBy.find((item) => item.column === key)?.order,
					handleSortingChange,
				)}
			</div>
		);
	...
	const getGroupsData = async () => {
		const dataFilters = Object.values(filters).map((item) => {
			if (item.columnField === 'type') {
				item.value = (item.value as string[]).map((tp) =>
			...
			await Promise.all(
				groups.map(async (group) => {
					try {
						...
						console.error(
							`Failed to fetch tree count for group ${group.id}`,
							error,
						);
						counts[group.id] = {
							mapped_trees: 0,
	...
	const columns: TableColumnsType<Group> = [
		...
		},
		{
			...
			...getColumnSelectedItemFilter({
				...
				filters,
				handleSetFilters,
				options: organizationTypes.map((item) => item.label.toUpperCase()),
			}),
		},
		{
					...
					<Link
						...
						onClick={(e) => {
							...
							if (hostname === 'localhost' || hostname === '127.0.0.1') {
								...
								window.open(
									'https://' + hostname + '/csr/dashboard/' + record.id,
								);
							}
						}}
						style={{
					...
					>
						<Link
							...
							onClick={(e) => {
								...
								if (hostname === 'localhost' || hostname === '127.0.0.1') {
									window.open(
										'http://' + host + '/dashboard/' + record.name_key,
									);
								} else {
									window.open(
										'https://' + hostname + '/dashboard/' + record.name_key,
									);
								}
							}}
							style={{
						...
						>
							<Dashboard fontSize="small" aria-label="Go to Dashboard" />
							{/* Dashboard */}
							{/* {totalTrees} */}
						</Link>
					</Tooltip>
	...
	const userGroupMapping = useAppSelector(
		(state: RootState) => state.userGroupsData,
	);
	const data = Object.entries(userGroupMapping).filter(
		([key, value]) => value.failed !== 0,
	);
		...
		<>
			...
			>
				...
				>
					...
					>
						<Badge
							badgeContent={Object.keys(filteredUserGroupMapping).length}
							color="error"
						>
							<ErrorIcon />
						</Badge>
			...
			<Dialog open={groupCombineModal} maxWidth="md">
				...
				<DialogContent>
					<DialogContentText>
						<CombineGroupForm
							...
							onDeleteSecondaryChange={(value) => {
								setDeleteSecondary(value);
							}}
						/>
					</DialogContentText>
				</DialogContent>
				<DialogActions>
					<Button
						variant="outlined"
						onClick={handleCancelCombineGroup}
						color="error"
					>
						Cancel
					</Button>
