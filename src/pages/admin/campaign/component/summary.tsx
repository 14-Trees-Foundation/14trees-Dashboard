...
import {
	...
	VolunteerActivism as DonorsIcon,
		...
		<>
			...
			{donors && donors.length > 0 && (
				<Box sx={{ textAlign: 'center', mt: 8, pb: 4 }}>
					<Typography variant="h4" color="#1f3625" sx={{ mb: 2 }}>
						<DonorsIcon
							sx={{ verticalAlign: 'middle', marginRight: 1, color: '#4CAF50' }}
						/>
						Donors
					</Typography>
						...
						<Table>
							<TableHead>
								<TableRow>
									...
									<TableCell sx={tableHeaderStyle}>Receipt No.</TableCell>
								</TableRow>
							</TableHead>
							<TableBody>
								{donors.map((donor, index) => (
									<TableRow key={donor.donationId} sx={tableRowStyle}>
										<TableCell>{index + 1}</TableCell>
										<TableCell>{donor.name || '—'}</TableCell>
										<TableCell>
											{donor.amount != null ? formatAmount(donor.amount) : '—'}
										</TableCell>
										<TableCell>{donor.donationReceiptNumber || '—'}</TableCell>
									</TableRow>
								))}
							</TableBody>
						</Table>
			...
			{champions && champions.length > 0 && (
				<Box sx={{ textAlign: 'center', mt: 12 }}>
					<Typography variant="h4" color="#1f3625" sx={{ mb: 2 }}>
						<ChampionIcon
							sx={{ verticalAlign: 'middle', marginRight: 1, color: '#FFC107' }}
						/>
						Campaign Champions
					</Typography>
							...
							<TableBody>
								{champions.map((champion, index) => (
									<TableRow key={index} sx={tableRowStyle}>
										...
										<TableCell>
											{champion.referralDonationsCount || 0}
										</TableCell>
									</TableRow>
								))}
							</TableBody>
