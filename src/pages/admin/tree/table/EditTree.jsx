...
function EditTree({ row, openeditModal, handleCloseEditModal, editSubmit }) {
	...
	const { getPlantTypes } = bindActionCreators(
		plantTypeActionCreators,
		dispatch,
	);
	...
	useEffect(() => {
		const fetchDetails = async () => {
			...
			setEventName(row.description || '');

			if (row.mapped_to_user) {
				const userResp = await apiClient.getUsers(0, 1, [
					{
						columnField: 'id',
						operatorValue: 'equals',
						value: row.mapped_to_user,
					},
				]);
				...
				const userResp = await apiClient.getUsers(0, 1, [
					{
						columnField: 'id',
						operatorValue: 'equals',
						value: row.sponsored_by_user,
					},
				]);
				if (userResp.results.length === 1)
					setSponsoredUser(userResp.results[0]);
				else setSponsoredUser(null);
			} else setSponsoredUser(null);

			if (row.mapped_to_group) {
				const groupResp = await apiClient.getGroups(0, 1, [
					{
						columnField: 'id',
						operatorValue: 'equals',
						value: row.mapped_to_group,
					},
				]);
				if (groupResp.results.length === 1)
					setReservedGroup(groupResp.results[0]);
				...
				const groupResp = await apiClient.getGroups(0, 1, [
					{
						columnField: 'id',
						operatorValue: 'equals',
						value: row.sponsored_by_group,
					},
				]);
				if (groupResp.results.length === 1)
					setSponsoredGroup(groupResp.results[0]);
	...
	if (plantTypesData) {
		...
		if (!Object.hasOwn(plantTypesMap, formData.plant_type_id)) {
			plantTypesMap[formData.plant_type_id] = {
				id: formData.plant_type_id,
				name: formData.plant_type,
			};
		}
				...
				<Grid container spacing={2}>
						...
						<Autocomplete
							...
							onChange={(event, value) => {
								if (value !== null)
									setFormData((prevState) => ({
										...prevState,
										plant_type_id: value.id,
									}));
							}}
							value={
								plantTypeName === '' &&
								Object.hasOwn(plantTypesMap, formData.plant_type_id)
									? plantTypesMap[formData.plant_type_id]
									: null
							}
					...
					<Grid item xs={6}>
						<Autocomplete
							...
							value={
								plotName === '' && Object.hasOwn(plotsMap, formData.plot_id)
									? plotsMap[formData.plot_id]
									: null
							}
							getOptionLabel={(option) => option.name}
